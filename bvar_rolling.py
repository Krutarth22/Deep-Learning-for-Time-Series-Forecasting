...
while cnt < 110:
    ...
    for l in range(lag-1):
        l+=1
        Xdum = pd.concat([Xdum, train.shift(l)], axis=1)
    X1 = np.matrix(Xdum.dropna()[:-1])
    
    
    #X1 = np.matrix(train.shift(1).dropna())
    ...
    for step in range(12):
        ...
        for l in range(lag-1):
            l+=1
            Xdum2 = pd.concat([Xdum2, test_new[-lag:].shift(l)], axis=1)
        X2 = np.matrix(Xdum2.dropna())
        
        #X2 = np.matrix(test_new[-lag:])
        X_test = np.c_[np.ones(len(X2)), X2]
        forecasts[step] = np.dot(X_test,A_post)[0,0]
        test_new.loc[step+lag] = np.array(np.dot(X_test,A_post))[0]
